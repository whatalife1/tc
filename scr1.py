...
def print_dots(stop_event):
    ...
    while not stop_event.is_set():
        ...
        time.sleep(0.5) # Slightly faster dots for better feedback
...
def main():
    ...
    try:
        run_git_command(['git', 'add', '.'], cwd=repository_root) # Use 'git add .' to stage all changes
    ...
    try:
        ...
        # Explicitly check if the target file is staged
        if source_file_name not in status_result.stdout or "new file:" not in status_result.stdout and "modified:" not in status_result.stdout:
            ...
            # If this happens, commit might fail or do nothing. We proceed but warn the user.
    ...
    try:
        ...
        if commit_result and commit_result.returncode == 0:
            ...
            # This block handles cases where git commit returns a non-zero exit code
    ...
    try:
        ...
        if pull_result.returncode != 0:
            if "Already up to date." in pull_result.stdout:
                ...
                # If no upstream, pull fails. We'll just try to push.
            else:
